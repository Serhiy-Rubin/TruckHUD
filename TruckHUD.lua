...
local sampev, inicfg, dlstatus, vkeys, ffi = require 'lib.samp.events', require 'inicfg', require('moonloader').download_status, require 'lib.vkeys', require("ffi")
...
local pair_mode, sms_pair_mode, report_text, pair_mode_id, pair_mode_name, BinderMode = false, '', '', 999, 'Nick_Name', true
				...
				if f then
					...
					if text ~= nil then
						if not string.find(text, tostring(thisScript().version)) then
							sampAddChatMessage(">> Вышло обновление для Truck HUD, версия от "..text..'. Текущая версия от '..thisScript().version, 0xFF2f72f7)
...
function doControl()
	if isKeyDown(vkeys[inifiles.Settings.Key1]) and (isTruckCar() or (isKeyDown(vkeys[inifiles.Settings.Key2] or pos[1] or pos[2] or pos[3]))) and not sampIsDialogActive() and not sampIsScoreboardOpen() then
		sampSetCursorMode(3)
		...
		for i = 1, 10 do
			...
			if drawClickableText(string_render, ((X / 2) - (renderGetFontDrawTextLength(font, string_render) / 2)), Y) then
				if i == 1 then
					...
					if script_run then ReadLog() end
				end
				if i == 2 then
					auto = not auto
					menu[i].run = auto
				...
				if i == 7 then sampSendChat("/truck mon") end
				if i == 8 then sampSendChat("/truck load "..GetGruz()) 	end
				if i == 9 then sampSendChat("/truck unload") 	end
				if i == 10 then sampSendChat("/truck trailer") end
			end
			if i == 4 and pair_mode and drawClickableText("{e30202}х", ((X / 2) + (renderGetFontDrawTextLength(font, menu[4][1].."   ") / 2)), Y) then
				pair_mode = false
				menu[4].run = false
			end
			Y = Y + plus
			if i == 6 then Y = Y + plus end
		end
	else
			...
			if dialogLine ~= nil and dialogLine[list + 1] ~= nil then
				...
				if str:find('Цвет подсветки напарника') then
					if dialogTextToList[list + 1] ~= nil then
						ShowDialog1(2, dialogTextToList[list + 1], inifiles.Settings.ColorPara, true, 'Settings', 'ColorPara')
					end
				end
				...
				if str:find('Размер') then
					if dialogTextToList[list + 1] ~= nil then
						ShowDialog1(2, dialogTextToList[list + 1], inifiles.Render.FontSize, false, 'Render', 'FontSize')
					end
				end
				if str:find('Стиль') then
					if dialogTextToList[list + 1] ~= nil then
						ShowDialog1(2, dialogTextToList[list + 1], inifiles.Render.FontFlag, false, 'Render', 'FontFlag')
					end
				end
...
function doPair()
	if pair_mode then
		if not sampIsPlayerConnected(pair_mode_id) or sampGetPlayerNickname(pair_mode_id) ~= pair_mode_name then
			pair_mode = false
			menu[4].run = false
			sampAddChatMessage('Напарник '..pair_mode_name..'['..pair_mode_id..']'..' вышел из игры. Режим пары выключен.', -1)
		end
	end
end
...
function doRenderStats()
	...
	if timer_secc >= 177 and workload == 0 and isTruckCar() and inifiles.Settings.Stop then setGameKeyState(6, 255) end
	timer_min, timer_sec = math.floor(timer_secc / 60), timer_secc % 60
	strok = (timer_secc >= 0 and (workload == 1 and string.format('{%s}До разгрузки {%s}%d:%02d', inifiles.Render.Color1, (timer_secc <= 10 and "b50000" or inifiles.Render.Color2), timer_min, timer_sec) or string.format('{%s}До загрузки {%s}%d:%02d', inifiles.Render.Color1, (timer_secc <= 10 and "b50000" or inifiles.Render.Color2), timer_min, timer_sec) ) or (workload == 1 and string.format('{%s}Можно разгружать', inifiles.Render.Color1) or string.format('{%s}Можно загружать', inifiles.Render.Color1)))
	if auto then
		if control then
			local delta = getMousewheelDelta()
			if delta ~= 0 then ChangeCena(delta) end
		end
		local autoColor = (autoh and inifiles.Render.Color2 or "d90b0b")
		str = (inifiles.Price[(workload == 1 and 'UnLoad' or 'Load')] ~= 0 and " {"..autoColor.."}["..(workload == 1 and 'Un' or '').."Load: "..inifiles.Price[(workload == 1 and 'UnLoad' or 'Load')].."] " or " {"..autoColor.."}["..(workload == 1 and 'Un' or '').."Load] ")
		if os.difftime(os.time(), timer) > 180 and autoh then
			if workload == 1 then
				if unload_location then
					...
					local dcena = (prices_mon[dp[dport]..ds] + prices_mon[current_warehouse]) - prices_3dtext[current_warehouse]
				...
				if load_location then
					if inifiles.Price.Load ~= 0 then
						if ( price_frozen and tonumber(prices_3dtext[current_warehouse]) == tonumber(inifiles.Price.Load) ) or ( not price_frozen and tonumber(prices_3dtext[current_warehouse]) <= tonumber(inifiles.Price.Load) ) then
		...
		if drawClickableText(str, (X + renderGetFontDrawTextLength(font, strok)), Y) then
			if autoh then 
				if workload == 1 then inifiles.Price.UnLoad = 0 else inifiles.Price.Load = 0 end
				inicfg.save(inifiles, AdressIni)
			else
				delay.load = 0
				delay.unload = 0
		...
		if isKeyDown(vkeys[inifiles.Settings.Key1]) and (isTruckCar() or isKeyDown(90)) then
			if drawClickableText("+", (X + renderGetFontDrawTextLength(font, strok) + (renderGetFontDrawTextLength(font, str) / 3)), (Y - height)) then ChangeCena(1) end
			if drawClickableText("-", (X + renderGetFontDrawTextLength(font, "+"..strok) + (renderGetFontDrawTextLength(font, str) / 2) ), (Y - height)) then ChangeCena(0) end
		end	
	end
	drawClickableText(strok, X, Y)
	string_render, Y = string.format('{%s}Скилл: {%s}%s [%s%%] [%s]', c1, c2, inifiles.Trucker.Skill, inifiles.Trucker.ProcSkill, inifiles.Trucker.ReysSkill), Y + height + down
	drawClickableText(string_render, X, Y)
	string_render, Y = string.format('{%s}Ранг: {%s}%s [%s%%] [%s]', c1, c2, inifiles.Trucker.Rank, inifiles.Trucker.ProcRank, inifiles.Trucker.ReysRank), Y + height
	drawClickableText(string_render, X, Y)
	if inifiles.Settings.Style then
		string_render, Y = string.format('{%s}Зарплата: {%s}%d/%d', c1, c2, log.ZPH, inifiles.Trucker.MaxZP), Y + height + down
		drawClickableText(string_render, X, Y)
		...
		if inifiles.Stats.Hour then
			...
			drawClickableText(string_render, X, Y)
			string_render, Y = string.format('{%s} Зарплата: {%s}%d/%d', c1, c2, log.ZPH, inifiles.Trucker.MaxZP), Y + height
			drawClickableText(string_render, X, Y)
	...
	if delay.skill == -1 then delay.skill = 1 end
end
...
function doRenderMon()
	...
	local height = renderGetFontDrawHeight(font)
		if mon_secund ~= -2 then 
			...
			if A2 >= A1 then stimer = A1 else stimer = A2 end
		...
		if (mon_secund >= 20 or mon_secund == -2) and inifiles.Settings.MonDownload then
			...
			downloadUrlToFile("http://truck.hud.xsph.ru/"..server, fpath, function(id, status, p1, p2)
				if status == dlstatus.STATUS_ENDDOWNLOADDATA then
					local f = io.open(fpath, 'r')
					if f then
						local text = f:read('*a')
						if string.find(text, "%[LS N:%d+ Y:%d+ L:%d+%] %[1 N:%d+ Y:%d+ L:%d+%] %[2 N:%d+ Y:%d+ L:%d+%] %[SF N:%d+ Y:%d+ L:%d+%] (%d+)") then
							G1 = string.match(text, "%[LS N:%d+ Y:%d+ L:%d+%] %[1 N:%d+ Y:%d+ L:%d+%] %[2 N:%d+ Y:%d+ L:%d+%] %[SF N:%d+ Y:%d+ L:%d+%] (%d+)")
							G2 = os.difftime(os.time(), G1)		 -- Из сервера
							G3 = os.difftime(os.time(), mon_ctime) -- из чата
							if tonumber(G3) > tonumber(G2) then
								S1 = mon_time
								prices_mon.n1, prices_mon.n2, prices_mon.y1, prices_mon.y2, prices_mon.l1, prices_mon.l2, prices_mon.lsn, prices_mon.lsy, prices_mon.lsl, prices_mon.sfn, prices_mon.sfy, prices_mon.sfl, mon_time = string.match(text, "%[LS N:(%d+) Y:(%d+) L:(%d+)%] %[1 N:(%d+) Y:(%d+) L:(%d+)%] %[2 N:(%d+) Y:(%d+) L:(%d+)%] %[SF N:(%d+) Y:(%d+) L:(%d+)%] (%d+)")
								if S1 ~= mon_time then mon_life = os.time() end
							end
						end
					io.close(f)
					end
	...
	if drawClickableText(string1, X, Y) then
		sampSendChat("/jf chat Еду на Нефтезавод №1. До цели: "..math.ceil(getDistanceBetweenCoords3d(pX, pY, pZ, 256.02127075195, 1414.8492431641, 10.232398033142))..' м.')
	end
	...
	if drawClickableText(string2, (X + renderGetFontDrawTextLength(font, string1)), Y) then
		sampSendChat("/jf chat Еду на Нефтезавод №2. До цели: "..math.ceil(getDistanceBetweenCoords3d(pX, pY, pZ, -1046.7521972656, -670.66937255859, 31.885597229004))..' м.')
	end
	Y = Y + height
	local string1 = string.format("  {%s}У1: {%s}%03d", c1, c2, prices_mon.y1)
	if drawClickableText(string1, X, Y) then
		sampSendChat("/jf chat Еду на Склад Угля №1. До цели: "..math.ceil(getDistanceBetweenCoords3d(pX, pY, pZ, 833.04681396484, 864.35931396484, 12.277567863464))..' м.')
	end
	local string2 = string.format(" {%s}У2: {%s}%03d", c1, c2, prices_mon.y2)
	if drawClickableText(string2, (X + renderGetFontDrawTextLength(font, string1)), Y) then
		sampSendChat("/jf chat Еду на Склад Угля №2. До цели: "..math.ceil(getDistanceBetweenCoords3d(pX, pY, pZ, -1872.8674316406, -1720.0148925781, 21.322338104248))..' м.')
	end
	Y = Y + height
	local string1 = string.format("  {%s}Л1: {%s}%03d", c1, c2, prices_mon.l1)
	if drawClickableText(string1, X, Y) then
		sampSendChat("/jf chat Еду на Лесопилку №1. До цели: "..math.ceil(getDistanceBetweenCoords3d(pX, pY, pZ, -448.91455078125, -65.951385498047, 58.959014892578))..' м.')
	end
	local string2 = string.format(" {%s}Л2: {%s}%03d", c1, c2, prices_mon.l2)
	if drawClickableText(string2, (X + renderGetFontDrawTextLength(font, string1)), Y) then
		sampSendChat("/jf chat Еду на Лесопилку №2. До цели: "..math.ceil(getDistanceBetweenCoords3d(pX, pY, pZ, -1978.8649902344, -2434.9421386719, 30.192840576172))..' м.')
	end
	Y = Y + height
	...
	if drawClickableText(string, X, Y) then
		if workload ~= 0 then
			sampSendChat("/jf chat Везу "..what_is_uploaded[current_load].." в Порт ЛС. До цели: "..math.ceil(getDistanceBetweenCoords3d(pX, pY, pZ, 2509.04296875, -2231.4343261719, 12.920718193054))..' м.')
		else
			sampSendChat("/jf chat Еду в Порт ЛС. До цели: "..math.ceil(getDistanceBetweenCoords3d(pX, pY, pZ, 2509.04296875, -2231.4343261719, 12.920718193054))..' м.')
		end
	end
	Y = Y + (height * 2)
	if control and workload == 1 then
	...
	if drawClickableText(string, X, Y) then
		if workload ~= 0 then
			sampSendChat("/jf chat Везу "..what_is_uploaded[current_load].." в Порт СФ. До цели: "..math.ceil(getDistanceBetweenCoords3d(pX, pY, pZ, -1733.1876220703, 120.08413696289, 3.1192970275879))..' м.')
		else
			sampSendChat("/jf chat Еду в Порт СФ. До цели: "..math.ceil(getDistanceBetweenCoords3d(pX, pY, pZ, -1733.1876220703, 120.08413696289, 3.1192970275879))..' м.')
		end
	end
	if control then
		Y = Y + (height * 2)
...
function doRenderBind()
	...
	if script_run and BindText ~= nil and inifiles.Settings.Binder and control or pos[3] then
		...
		local plus = (renderGetFontDrawHeight(font) + (renderGetFontDrawHeight(font) / 10))
		if not pair_mode and not BinderMode then BinderMode = true end
		if drawClickableText(string.format('{%s}[ %sРация {%s}%s{%s}]', inifiles.Render.Color2, (BinderMode and '{12a61a}' or '{ff0000}'), inifiles.Render.Color2, (not BinderMode and '| {12a61a}CMC ' or (pair_mode and '| {ff0000}CMC ' or '')), inifiles.Render.Color2), X, Y - renderGetFontDrawHeight(font)) then 
			BinderMode = not BinderMode
		end
		if drawClickableText('{'..inifiles.Render.Color2.."}[Смена позиции]", X, Y) then pos[3] = true end
		local List, string = 0, ''
		for i = 1, #EditLine do 
			if EditLine[i] ~= nil then
				...
				List = List + 1
				Y = Y + plus
				if drawClickableText(string, X, Y) then sampSendChat((BinderMode and '/jf chat ' or '/sms '..pair_mode_id..' ')..string) end
...
function ShowDialog1(int, dtext, dinput, string_or_number, ini1, ini2)
	...
	if int == 1 then
		...
		if script_run then
			...
			if inifiles.Settings.Binder then dialogLine[#dialogLine + 1] = "Сменить позицию биндера\t" end
		end

		dialogLine[#dialogLine + 1] = 'Компактная статистика\t'..(inifiles.Settings.Style == true and "{59fc30}ON" or "{ff0000}OFF")
		if not inifiles.Settings.Style then
			dialogLine[#dialogLine + 1] = 'Статистика за час\t'..(inifiles.Stats.Hour == true and "{59fc30}ON" or "{ff0000}OFF")
			dialogLine[#dialogLine + 1] = 'Статистика за сутки\t'..(inifiles.Stats.Day == true and "{59fc30}ON" or "{ff0000}OFF")
		end
		...
		dialogLine[#dialogLine + 1] = 'Режим авто загрузки/разгрузки\t'..(inifiles.Settings.AutoOFF == true and "{59fc30}Разовая" or "{59fc30}Постоянная")

		dialogLine[#dialogLine + 1] = 'Задержка перед авто-загрузкой\t'..(inifiles.Settings.AutoWait == true and "{59fc30}ON" or "{ff0000}OFF")

		dialogLine[#dialogLine + 1] = 'Остановка фуры после разгрузки\t'..(inifiles.Settings.Stop == true and "{59fc30}ON" or "{ff0000}OFF")

		dialogLine[#dialogLine + 1] = 'Авто Доклады в рацию\t'..(inifiles.Settings.Report == true and "{59fc30}ON" or "{ff0000}OFF")

		dialogLine[#dialogLine + 1] = 'Доклады от?\t'..(inifiles.Settings.girl == true and "{59fc30}Женщины" or "{59fc30}Мужчины")

		dialogLine[#dialogLine + 1] = 'Авто Отправка мониторинга в чат\t'..(inifiles.Settings.ChatDoklad and "{59fc30}ON" or "{ff0000}OFF")

		dialogLine[#dialogLine + 1] = (pair_mode and "Режим пары\t{59fc30}"..pair_mode_name.."["..pair_mode_id.."]" or "Режим пары\t{ff0000}OFF")

		dialogLine[#dialogLine + 1] = 'Авто Доклады в SMS (режим пары)\t'..(inifiles.Settings.SMSpara == true and "{59fc30}ON" or "{ff0000}OFF")

		dialogLine[#dialogLine + 1] = 'Подсветка напарника в чате (режим пары)\t'..(inifiles.Settings.LightingPara == true and "{59fc30}ON" or "{ff0000}OFF")

		dialogLine[#dialogLine + 1] = 'Выделение Портов/Складов/Цен в докладах\t'..(inifiles.Settings.highlight_jf == true and "{59fc30}ON" or "{ff0000}OFF")

		dialogLine[#dialogLine + 1] = 'Выделение цены текущего груза в порту\t'..(inifiles.Settings.LightingPrice == true and "{59fc30}ON" or "{ff0000}OFF")

		dialogLine[#dialogLine + 1] = 'Показывать игроков с /clist 0 на карте\t'..(inifiles.Settings.markers == true and "{59fc30}ON" or "{ff0000}OFF")

		dialogLine[#dialogLine + 1] = 'Скрывать чат профсоюза\t'..(inifiles.Settings.ChatOFF == true and "{59fc30}ON" or "{ff0000}OFF")

		dialogLine[#dialogLine + 1] = 'Убрать тюнинг колес с фур\t'..(inifiles.Settings.Tuning == false and "{59fc30}ON" or "{ff0000}OFF")

		dialogLine[#dialogLine + 1] = 'Получение мониторинга с Хостинга\t'..(inifiles.Settings.MonDownload == true and "{59fc30}ON" or "{ff0000}OFF")
		
		dialogLine[#dialogLine + 1] = 'Цвет подсветки напарника\t{'..inifiles.Settings.ColorPara.."}"..inifiles.Settings.ColorPara -- 6
		dialogTextToList[#dialogLine] = "{FFFFFF}Введите новый цвет в HEX\nПодобрать цвет можно через браузер\nЧтобы скопировать ссылку введите /truck url"
	...
	if int == 5 then
		...
		local text = ""
		for k,v in pairs(log_files) do text = text..log_files[k].."\n" end
	...
	if int == 7 then
		sampShowDialog(222, "Truck-HUD: Биндер", "{FFFFFF}Поддерживает замены\n!НикПары - Заменится ником напарника\n!ИдПары - Заменится на ID напарника\n!КД - Заменится на время до загрузки/разгрузки", "Сохранить", "Закрыть", 1)
		if dtext == 2 then
			d[7] = true
		end
	end
	if int == 8 then
		sampShowDialog(222, "Truck-HUD: Режим пары", "\t{FFFFFF}Введите ID напарника\nЕму будут отсылаться SMS сообщения о ваших загрузках/разгрузках", "Выбрать", "Закрыть", 1)
	end
end
--[[
function FindSklad()
...
function sampev.onServerMessage(color, message)
	...
	if string.find(message, " (.*)<(.*)>: %[ЛС Н:(%d+) У:(%d+) Л:(%d+)%] %[1 Н:(%d+) У:(%d+) Л:(%d+)%] %[2 Н:(%d+) У:(%d+) Л:(%d+)%] %[CФ Н:(%d+) У:(%d+) Л:(%d+)%]") then
		mon_life = os.time()
		mon_ctime = os.time()
		if (string.find(message, "Купил") or string.find(message, "Продал")) then
			nick, rank, prices_smon.lsn, prices_smon.lsy, prices_smon.lsl, prices_smon.n1, prices_smon.y1, prices_smon.l1, prices_smon.n2, prices_smon.y2, prices_smon.l2, prices_smon.sfn, prices_smon.sfy, prices_smon.sfl, _ = string.match(message, " (.*)<(.*)>: %[ЛС Н:(%d+) У:(%d+) Л:(%d+)%] %[1 Н:(%d+) У:(%d+) Л:(%d+)%] %[2 Н:(%d+) У:(%d+) Л:(%d+)%] %[CФ Н:(%d+) У:(%d+) Л:(%d+)%] %[(.*)%]")
		else
			nick, rank, prices_smon.lsn, prices_smon.lsy, prices_smon.lsl, prices_smon.n1, prices_smon.y1, prices_smon.l1, prices_smon.n2, prices_smon.y2, prices_smon.l2, prices_smon.sfn, prices_smon.sfy, prices_smon.sfl = string.match(message, " (.*)<(.*)>: %[ЛС Н:(%d+) У:(%d+) Л:(%d+)%] %[1 Н:(%d+) У:(%d+) Л:(%d+)%] %[2 Н:(%d+) У:(%d+) Л:(%d+)%] %[CФ Н:(%d+) У:(%d+) Л:(%d+)%]")
		end
	...
	if string.find(message, " Извините, мы вас немного задержим, нужно подготовить груз. Осталось (%d+) секунд") then
		local S1 = string.match(message, " Извините, мы вас немного задержим, нужно подготовить груз. Осталось (%d+) секунд")
		if tonumber(S1) > 3 then delay.load = 0 delay.unload = 0 end
	end

	if message == " У вас недостаточно денег" or message == " Нужно находиться у склада" or message == " Нужно находиться в порту" or message == " У вас нет продуктов" or message == " Вы прибыли без прицепа" then
		delay.load = 0
		delay.unload = 0
		if auto then
			autoh = false
	...
	if message == " Вы не в служебной машине. Нужно быть водителем" or message == " Вы должны находиться в порту, или на складе" or message == ' Вы должны устроиться на работу дальнобойщика' then 
	...
	if message == " Сообщение доставлено" or message == " Игрок оффлайн" or message == " Введите: /sms [playerid / phonenumber] [текст]" or message == " Телефон вне зоны доступа сети" then
		delay.sms = 0
	end
	...
	if string.find(message, "Загружено %d+ груза, на сумму (%d+) вирт. Скидка: %d+ вирт") and isTruckCar() then
		...
		delay.load = 0 
		if script_run then 
			if inifiles.Settings.ChatDoklad then delay.chatMon = -1 end
			delay.mon = 1
		end
		workload = 1
		autoh = true
		if inifiles.Settings.AutoOFF then auto = false end
	end
	...
	if message == " Не флуди!" then 
 		if delay.skill == 2 then delay.skill = 1 end
 		if delay.load == 2 then delay.load = 1 end
 		if delay.unload == 2 then delay.unload = 1 end
 		if delay.sms == 2 then delay.sms = 1 end
 		if delay.chat == 2 then delay.chat = 1 end
 		if delay.chatMon == 2 then delay.chatMon = 1 end
 		if delay.dir == 2 then delay.dir = 1 end
	end
	if message == ' У вас нет телефонного справочника!' then
		delay.dir = 0
	end
	if string.find(message, " Вы арендовали транспортное средство") and isTruckCar() then WriteLog(message, 3) end
	if string.find(message, " Вы заплатили штраф (%d+) вирт, Офицеру (%g+)") then WriteLog(message, 4) end
	if string.find(message, " Вашу машину отремонтировал%(а%) за (%d+) вирт, Механик (%g+)") and isTruckCar() then WriteLog(message, 5) end
	if string.find(message, " Автомеханик (%g+) заправил ваш автомобиль на 300 за (%d+) вирт") and isTruckCar() then WriteLog(message, 6) end
	if string.find(message, " Машина заправлена, за: (%d+) вирт") and isTruckCar() then WriteLog(message, 7) end
	if string.find(message, " Вы купили канистру с 50 литрами бензина за (%d+) вирт") and isTruckCar() then WriteLog(message, 8) end
end
...
function sampev.onShowDialog (DdialogId, Dstyle, Dtitle, Dbutton1, Dbutton2, Dtext)
		...
		if SkillP ~= nil then
			...
			RankP = tonumber(RankP)		
			 if inifiles.Trucker.ProcSkill ~= SkillP then
				...
				local gruzs = ( Skill < 10 and 10000 or ( Skill < 20 and 20000 or ( Skill < 30 and 30000 or ( Skill < 40 and 40000 or ( Skill >= 40 and 50000)))))
	...
	if DdialogId == 22 and Dstyle == 0 and string.find(Dtext, "Заводы") then
		...
		for k,v in pairs(prices_mon) do if string.find(tostring(prices_mon[k]), "99") then prices_mon[k] = tonumber(prices_mon[k]) + 1 end end
		local data = string.format("[LS N:%d Y:%d L:%d] [1 N:%d Y:%d L:%d] [2 N:%d Y:%d L:%d] [SF N:%d Y:%d L:%d]", prices_mon.n1, prices_mon.n2, prices_mon.y1, prices_mon.y2, prices_mon.l1, prices_mon.l2, prices_mon.lsn, prices_mon.lsy, prices_mon.lsl, prices_mon.sfn, prices_mon.sfy, prices_mon.sfl)
		downloadUrlToFile("http://truck.hud.xsph.ru/index.php?server="..server.."&text="..data)
		if delay.chatMon == -1 then
			SendMonText = string.format("[ЛС Н:%d У:%d Л:%d] [1 Н:%d У:%d Л:%d] [2 Н:%d У:%d Л:%d] [CФ Н:%d У:%d Л:%d]", (prices_mon.lsn / 100), (prices_mon.lsy / 100), (prices_mon.lsl / 100), (prices_mon.n1 / 100), (prices_mon.y1 / 100), (prices_mon.l1 / 100), (prices_mon.n2 / 100), (prices_mon.y2 / 100), (prices_mon.l2 / 100), (prices_mon.sfn / 100), (prices_mon.sfy / 100), (prices_mon.sfl / 100))
			delay.chatMon = 1
		end
		mon_secund = -2
		if script_run then return false end
	end
	...
	if delay.dir ~= 0 then
		...
		if string.find(Dtitle, "Работы") and delay.dir == 3 then
			lua_thread.create(function()
				repeat wait(0) until delay.dir == 4
				wait(150)
...
function sampev.onCreate3DText(id, color, position, distance, testLOS, attachedPlayerId, attachedVehicleId, text) -- f3d1
	lua_thread.create(function(id, color, position, distance, testLOS, attachedPlayerId, attachedVehicleId, textt)
		for k,v in pairs(find_3dText) do
			if string.find(text, v) then
				...
				if text:find('Порт') then
					...
					prices_3dtext[k..'n'], prices_3dtext[k..'y'], prices_3dtext[k..'l'] = string.match(text, v)
					for k,v in pairs(prices_3dtext) do if string.find(tostring(prices_3dtext[k]), "99") then prices_3dtext[k] = tonumber(prices_3dtext[k]) + 1 end end
					local port = (text:find('ЛС') and 'ЛС' or 'СФ')
					local ctext = string.format('Порт %s\nНефть: 0.%s\nУголь: 0.%s\nДерево: 0.%s ', port, prices_3dtext[k..'n'], prices_3dtext[k..'y'], prices_3dtext[k..'l'])
					current_warehouse = (current_load == 1 and k..'n' or (current_load == 2 and k..'y' or (current_load == 3 and k..'l' or '')))
					repeat wait(0) until sampIs3dTextDefined(id)
					...
					sampCreate3dTextEx(id, ctext, 0xFFFFFF00, position.x, position.y, position.z, distance, testLOS, attachedPlayerId, attachedVehicleId)
				else
	...
	end, id, color, position, distance, testLOS, attachedPlayerId, attachedVehicleId, text)
end
...
function WriteLog(message, Log)
	...
	if Log == 1 then 
		...
		if texts_of_reports[current_warehouse] ~= nil then
			...
			report_text = (not inifiles.Settings.girl and 'Загрузился' or 'Загрузилась').." на "..sklad.." по "..cena
			sms_pair_mode = report_text
			if inifiles.Settings.Report then delay.chat = 1 end
			if pair_mode and inifiles.Settings.SMSpara then delay.sms = 1 end
		end
	...
	if Log == 2 then 
		...
		local Z3 = Z1 - Z2
			if inifiles.Trucker.MaxZP > (log.ZPH + Z2) then
				if workload == 1 then
					WriteText = os.date("%X",os.time()) .. " | Загрузка " .. Z3 .. "\n" .. os.date("%X",os.time()) .. " | Разгрузка " .. Z1 .."\n" .. os.date("%X",os.time()) .. " | Заработано " .. Z2 .. "\n"
					Write = 2
				else
					WriteText = os.date("%X",os.time()) .. " | Разгрузка " .. Z1 .."\n" .. os.date("%X",os.time()) .. " | Заработано " .. Z2 .. "\n"
					Write = 1
				end
			else
				...
				local Z5 = string.match(Z4, "-(.*)")
					if workload == 1 then
						WriteText = os.date("%X",os.time()) .. " | Загрузка 0\n" .. os.date("%X",os.time()) .. " | Разгрузка " .. Z5 .."\n" .. os.date("%X",os.time()) .. " | Заработано " .. Z5 .. "\n"
						Write = 2
					else
						WriteText = os.date("%X",os.time()) .. " | Разгрузка " .. Z5 .."\n" .. os.date("%X",os.time()) .. " | Заработано " .. Z5 .. "\n"
						Write = 1
					end
				end
			end
...
local logs = { arenda = 0, arendah = 0, zagruzka = 0, zagruzkah = 0, razgruzka = 0, razgruzkah = 0, pribil = 0, pribilh = 0, shtraf = 0, shtrafh = 0, repair = 0, repairh = 0, refill = 0, refillh = 0, reys = 0, reysh = 0, kanistr = 0, kanistrh = 0 }
	if is == nil then adress = AdressLogFolder .. os.date("%d.%m.%Y") .. ".txt" else adress = is end
	...
	if is == nil then
		...
		log.ZP = logs.pribil
		log.PribH = logs.razgruzkah - logs.zagruzkah - logs.shtrafh - logs.repairh - logs.refillh - logs.arendah - logs.kanistrh
		...
		local prib = logs.razgruzka - logs.zagruzka - logs.shtraf - logs.repair - logs.refill - logs.arenda - logs.kanistr
		local text = "{FFFFFF}     Статистика за сутки\n Рейсов сделано: "..logs.reys.."\n Зарплаты получено: "..logs.pribil.."$\n Прибыль: "..prib.."$\n\n\tЗатраты\n Аренда: "..logs.arenda.."$\n Заправка: "..logs.refill.."$\n Починка: "..logs.repair.."$\n Канистры: "..logs.kanistr.."$\n Штрафы: "..logs.shtraf.."$\n Все затраты: "..(logs.arenda + logs.refill + logs.repair + logs.kanistr + logs.shtraf).."$"
...
function sampev.onSendCommand(cmd)
	...
	if command ~= nil and params ~= nil and command:lower() == "truck" then
		if params:lower() == " ad" then
			...
			inicfg.save(inifiles, AdressIni)
			sampAddChatMessage(string.format('Уведомления об обновлениях Truck HUD %s', (inifiles.Settings.ad and 'включены' or 'выключены')), 0xFF2f72f7)
			return false
		end
		...
		if params:lower() == " chat" then
			inifiles.Settings.Report = not inifiles.Settings.Report
			sampAddChatMessage(string.format('Авто Доклад в рацию %s', (inifiles.Settings.Report and 'активирован' or 'деактивирован')), 0xFF2f72f7)
			inicfg.save(inifiles, AdressIni)
			return false
		end
		...
		if params:lower():find(' mon (%d+)') then
			...
			return { string.format('/sms %s [ЛС H:%d У:%d Л:%d][1 H:%d У:%d Л:%d][2 H:%d У:%d Л:%d][CФ H:%d У:%d Л:%d]', id, (prices_mon.lsn / 100), (prices_mon.lsy / 100), (prices_mon.lsl / 100), (prices_mon.n1 / 100), (prices_mon.y1 / 100), (prices_mon.l1 / 100), (prices_mon.n2 / 100), (prices_mon.y2 / 100), (prices_mon.l2 / 100), (prices_mon.sfn / 100), (prices_mon.sfy / 100), (prices_mon.sfl / 100)) }
		end
	end
	antiflood = os.clock() * 1000
end
function sampev.onVehicleStreamIn(vehicleId, data) if inifiles ~= nil and not inifiles.Settings.Tuning and (data.type == 403 or data.type == 515) then data.modSlots[8] = 0 return {vehicleId, data} end end
        ...
        while isKeyDown(77) or isKeyDown(188) do
            ...
            if mapmode == 0 or mapmode == -1 then
                ...
                renderFontDrawText(
                    font10,
                    string.format(
                        "UPD: %s || РўРµРєСѓС‰Р°СЏ С†РµР»СЊ: %s   РќР°Р№РґРµРЅРѕ: %s   РђРєС‚РёРІРЅС‹С… РёСЃС‚РѕС‡РЅРёРєРѕРІ: %s   РњР°С€РёРЅ РІ Р±Р°Р·Рµ: %s",
                        count_next(),
